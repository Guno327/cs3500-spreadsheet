...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            visited.Add(name);
            //Traverse through every direct dependent
            foreach (string n in GetDirectDependents(name))
            {
                //If we manage to traverse to the originial node, there must be a cycle
                if (n.Equals(start))
                {
                    throw new CircularException();
                }
                //If we visit a node we havent yet, we must visit it in order to add all its dependents
                else if (!visited.Contains(n))
                {
                    Visit(start, n, visited, changed);
                }
            }
            //Once we have visited all the nodes that rely on this one,
            //We may add it to the list of nodes that need to be updated and return
